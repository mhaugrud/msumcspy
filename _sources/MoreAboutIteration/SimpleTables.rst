...
.. admonition:: Modify the program ...
   ...
   - In line 3, type ``, end = '\t'`` right after ``j``. Run and you will see a small multiplication table. ``end`` is an optional parameter for the ``print`` statement. It indicates what should be printed after the other items that are printed. The default is to print a new line character.
